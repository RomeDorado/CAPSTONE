    ...
    (session, args) => {
      ...
      request(options, function (error, response, body) {
        ...
        if(!response.body.d.facAccess){
            ...
            switch(args.intent){
                case 'get_next_class':
                    session.replaceDialog('/FacultySched/NextClass', {firstname: args.firstname, prof: args.prof, datetime: datetime});
                break;

                case 'get_faculty_room':
                    session.replaceDialog('/FacultySched/Room', {firstname: args.firstname, prof: args.prof, datetime: datetime});
                break;
            }            
